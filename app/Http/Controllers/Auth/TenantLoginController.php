...
class TenantLoginController extends Controller
{
     ...
     */
    public function __construct(RecaptchaService $recaptchaService, GoogleOAuthService $googleOAuthService)
    {
        $this->recaptchaService = $recaptchaService;
        $this->googleOAuthService = $googleOAuthService->forRedirectType('tenant');
    }
    ...
    public function handleGoogleCallback(Request $request)
    {
        try {
            if (!$request->has('code')) {
                return redirect('/tenant/login')->withErrors([
            ...
            \Log::info('Google login attempt for: ' . $googleUser->email);
            ...
            if ($tenantUser) {
                ...
                $tenant = $tenantUser->tenant;
                $isOwner = true;
                \Log::info('Google user found in central database as owner', [
            ...
            if ($tenant) {
                ...
                // If not found in central database, we need to check all tenant databases
                $tenants = Tenant::where('status', 'approved')->get();
                \Log::info('Searching in tenant databases for Google user', [
                    'email' => $googleUser->email,
                    'tenant_count' => $tenants->count()
                ]);
                ...
                foreach ($tenants as $potentialTenant) {
                    ...
                    if ($potentialUser) {
                        $user = $potentialUser;
                        $tenant = $potentialTenant;
                        \Log::info('Google user found in tenant database', [
                            'email' => $googleUser->email,
                            'tenant_slug' => $tenant->slug,
                            'role' => $user->role
                        ]);
            ...
            if (($tenantUser && !$tenant) || (!$tenantUser && !$user)) {
                return redirect('/tenant/login')->withErrors([
                    'email' => 'The Google account email does not match our records.',
                ]);
            }
            ...
            // If this is an owner (found in tenant_users), use that for authentication
            if ($isOwner) {
                \Log::info('Authenticating Google owner with central database record', [
                    'email' => $user->email,
                    'role' => $user->role
                ]);
                Auth::guard('tenant')->login($tenantUser);
                ...
                // For users only in tenant database, create a temporary TenantUser for authentication
                \Log::info('Creating temporary TenantUser for Google auth', [
                    'email' => $user->email,
                    'role' => $user->role,
                    'tenant_id' => $tenant->id
                ]);
                ...
                \Log::info('Successfully authenticated Google user', [
                    'email' => $tempTenantUser->email,
                    'id' => $tempTenantUser->id,
                    'tenant_id' => $tenant->id
                ]);
            ...
            // Redirect based on user role
            if ($user->role === 'owner') {
                \Log::info('Redirecting Google owner to dashboard', [
                    'role' => $user->role,
                    'tenant_slug' => $tenant->slug
                ]);
                return redirect('/' . $tenant->slug . '/dashboard');
            } else if ($user->role === 'judge') {
                \Log::info('Redirecting Google judge to dashboard', [
                    'role' => $user->role,
                    'tenant_slug' => $tenant->slug
                ]);
                return redirect('/' . $tenant->slug . '/judge-dashboard');
    ...
    public function debugOAuth()
    {
        ...
        $debug = [
            ...
            'routes' => [
                'redirect_route' => route('tenant.google.redirect'),
                'callback_route' => route('tenant.google.callback'),
            ],
